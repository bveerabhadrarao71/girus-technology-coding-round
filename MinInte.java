...
public class MinInte {
    ...
    private void executeLetcond(String srk) {
        ...
        String name = stats[0].trim();               // variable name and expression
        String expression = stats[1].trim();          
        int val = valexpression(expression);                  // then computes the values 
        vary.put(name, val);                       // and it can be stored in maps
    ...
    private boolean evaluate(String condition) {
        ...
        return false;  // in this no operator found and it will be in false condition
    }

    // Evaluate an arithmetic expression: + and - two terms...
    private int valexpression(String expression) {
        // evaluate addition or subtraction operation 
