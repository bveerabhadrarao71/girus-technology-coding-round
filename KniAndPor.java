...
public class KniAndPor {
    ...
    // Run a ktr from start to end 
    // it will be Returns a  number of steps, or -1 then if end is unreachable
    private static int ktr(int[][] grid, int[] start, int[] end) {
        ...
        // it will be starts intially at 0
        ...
        while (!queue.isEmpty()) {
            ...
            //  in this condn If we reached end, it will return the distance
            if (r == end[0] && p == end[1]) {
                return dist;
            }
